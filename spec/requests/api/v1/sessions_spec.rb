RSpec.describe 'Sessions', type: :request do
  ...
  describe 'POST /api/v1/auth/sign_in' do
    it 'valid credentials returns user' do
      ...
      expected_response = {
        'data' => {
          'id' => user.id, 'uid' => user.email, 'email' => user.email, 'allow_password_change' =>false, 'provider' => 'email', 'name' => nil, 'nickname' => nil, 'image' => nil
        }
      }
    ...
    it 'invalid password returns error message' do
      post '/api/v1/auth/sign_in', params: { email: user.email, password: 'another password' }, headers: headers
    ...
    it 'invalid email returns error message' do
      post '/api/v1/auth/sign_in', params: { email: 'linda @ .com', password: user.password }, headers: headers
